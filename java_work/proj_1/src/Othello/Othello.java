...
public class Othello extends JPanel implements MouseListener, ActionListener {
	...
	public Othello() throws IOException {
		...
		white = ImageIO.read(new File("C://siat_2023/java_work/proj_1/img/wh2.png"));
		whiteIcon = new ImageIcon(white);
		
		black = ImageIO.read(new File("C://siat_2023/java_work/proj_1/img/bk2.png"));
		...
		// 판 초반 세팅 	
		  ...
		  squarePanel[3][3].setState("white"); squarePanel[4][3].setState("black");
		  squarePanel[3][4].setState("black"); squarePanel[4][4].setState("white");
	...
	public boolean checkUp(int row, int column, String turn) {
		 ...
		 * 
		 * } break; } else if (state.equals("white") && turn.equals("white")) { for (int j
		 * = i - 1; j > row; j--) { squarePanel[j][column].getPiece().setIcon(whiteIcon);
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "턴 주게??":
			...
			int result = JOptionPane.showConfirmDialog(null, "턴 주게??", "알았어~",
		...
		case "포기??":
			...
			int giveUpResult = JOptionPane.showConfirmDialog(null, "포기??", "알았어~",
					JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
